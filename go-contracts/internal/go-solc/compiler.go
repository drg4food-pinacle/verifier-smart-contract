...
func (c *Compiler) compile(baseDir, contract string, opts []Option) (*output, error) {
	...
	// build src map
	srcMap, err := buildSrcMap(parentDir)
	if err != nil {
		return nil, err
	}
...
func (c *Compiler) run(baseDir string, in *input) (*output, error) {
	...
	// run solc
	args := []string{
		"--base-path", filepath.Dir(baseDir),
		"--standard-json",
	}

	// Add multiple --include-path flags
	for _, path := range allowPaths {
		args = append(args, "--include-path", path)
	}

	// Add multiple --allow-paths flags if needed
	args = append(args, "--allow-paths", baseDir)

	ex := exec.Command(c.solcAbsPath, args...)
	ex.Stdin = inputBuf
	ex.Stdout = outputBuf
